...
export default function BlindPullToggle({
  ...
  const [toggleDark, setToggleDark] = useState(() => controlledDark ?? true);
  ...
  // Sync with controlled isDark prop if provided
  useEffect(() => {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (cardWrapper)
      mo.observe(cardWrapper, { attributes: true, attributeFilter: ["class"] });
    return () => mo.disconnect();
  }, [controlledDark]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (propSize !== undefined) return;
    const el = scope.current;
    if (!el) return;
    const update = () => {
  ...
  const dotSize = Math.max(7, Math.round(size * 0.138)); // tune: raise the multiplier to enlarge the pull dot
  ...
  const cordTop = pageIsDark ? "rgba(255,255,255,0.4)" : "rgba(0,0,0,0.3)";
  const cordBottom = pageIsDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.1)";
  const dotBg = pageIsDark ? "rgba(255,255,255,0.8)" : "rgba(0,0,0,0.45)";
  ...
  return (
    <div
      ...
      style={showCardBackground ? { background: previewBg } : undefined}
      title={`Switch to ${toggleDark ? 'light' : 'dark'} mode (pull cord)`}
    >
      <motion.div
      ...
      >
        ...
        <div
          className="flex cursor-pointer flex-col items-center -mt-0.5"
          onClick={handleToggle}
          aria-hidden="true"
        >
          <div
